...
- v0_4_1 : 2023.6.1 : [ ](https://github.com/tokamak-network/tokamak-staking-v2/tree/TSV2_3_fast_withdraw)(current document)
  - modified event parameters ( Staked, Unstaked, Restaked, Withdraw) 
    - [link](/f3b340156ff0473c97942e6c209a83b8) 
```json
event Staked(uint32 _operatorIndex, uint32 _candidateIndex, address sender, uint256 amount, uint256 lton, address commissionTo, uint16 commission);
event Unstaked(uint32 _operatorIndex, uint32 _candidateIndex, address sender, uint256 amount, uint256 lton);
event Restaked(uint32 _operatorIndex, uint32 _candidateIndex, address sender, uint256 amount, uint256 lton);
event Withdrawal(uint32 _operatorIndex, uint32 _candidateIndex, address sender, uint256 amount);
```
  - add isCandidate parameter in re-stake, multi-restake, withdraw functions 
    - #F34~ #F39
  - deployed the changed parts on goerli 
- v0_4_2 : 2023.6.2  : [ ](https://github.com/tokamak-network/tokamak-staking-v2/tree/TSV2_3_fast_withdraw)(current document)
  - modified **Staked** event parameters  ***from commission to commissionLton***
```json
event Staked(uint32 _operatorIndex, uint32 _candidateIndex, address sender, uint256 amount, uint256 lton, address commissionTo, **uint256** **commissionLton**);
```
- **v0_6_1 : 2023.6.20  : **[** **](https://github.com/tokamak-network/tokamak-staking-v2/tree/TSV2_3_fast_withdraw)**(current document)**
  - rename Sequencer to L2Operator
  - change "OVM_Sequencer" to “OVM_TONStakingManager” 

# Repo 

- v0_6 :   [https://github.com/tokamak-network/tokamak-staking-v2/tree/TSV2_9_Rename](https://github.com/tokamak-network/tokamak-staking-v2/tree/TSV2_9_Rename)

# ABI 

- v0_6:  [https://github.com/tokamak-network/tokamak-staking-v2/tree/TSV2_9_Rename/artifacts/contracts](https://github.com/tokamak-network/tokamak-staking-v2/tree/TSV2_9_Rename/artifacts/contracts)
...
uint256 public **minimumDepositForL2Operator**;  // 초기 시퀀서의 최소 디파짓 금액,
...
uint32 public **indexOperators** ;  // 계속 증가만 함. 시퀀서 인덱스로 사용
...
**** amount[2] -> [amountOfstaker, ****amountOfOperator**~~**, amountOfDao, amountOfStosHolders**~~**]
**
...
event CreatedOptimismL2Operator(uint32 _index, address _operator, bytes32 _name, address addressManager, address l1Messenger, address l1Bridge, address l2ton, uint256 depositAmount);
...
event FastWithdrawalStaked(bytes32 fwHashMessage, uint32 layerIndex, address staker, uint256 amount, uint256 lton)
...
## F1. Create L2Operator ( Optimism Operator)  
...
function createOptimismL2Operator(
        ...
        external return (uint32)
```
- sample   
```javascript
const receipt = await(
	deployed.layer2Manager.connect(operator1).createOptimismL2Operator(
...
```
- comments 
  - You should approve the depositAmount of TON to Layer2Manager.
...
**info** = abi.encodePacked(addressManager, l1Bridge, l2Bridge, l2ton) 

**info **is composed of 8**0 bytes (addressManager address)| 20 bytes (**l1Bridge** address)| 20 bytes (**l2Bridge** address) | 20 bytes (l2ton address)** , it is 80 bytes.
```
- sample  
```javascript
...
```
- comments 
  - You should approve the stakeAmount of TON.
    - stakeAmount ≥ minimumDepositForCandidate
  - You will stake the stakeAmount of TON through createCandidate function. 
...
** bytes's format of **info (one of infos_) **: 
**info** = abi.encodePacked(operator, operatorIndex, commission) 
 
**info **is composed of **20 bytes (operator address) | 4 bytes (**operatorIndex**) | 2 bytes (**commission), it is 26 bytes.
...
## F5. View Operator’s info  

- contract : OptimismL2Operator
- Interface
```javascript
...
## F7. Check if the operator is already registered 
...
## F9. The total staked amount ( operator  + candidate )  
...
## F10. The total staked amount of operator  
...
## F12. The staked amount of special operator  
...
## F14. The staked amount of special operator's account
...
## F16. Total pending amount  by unstaking on a specific operator 
...
## F18. Total pending amount  by unstaking on a specific operator’s account 
...
## F20. Total pending amount of account on operator
...
## F22. Query accounts that have staked on a specific operator
...
## F26. Query the seigniorage amount to distribute to operators
...
## F27. Distribute the seigniorage amount to operators
...
## F28. Query the operator's seignorage query
...
function layerHoldings(uint32 layerKey_) external view returns (**Layer2.Layer2Holdings** memory)
 ...
 struct **Layer2Holdings** {
        uint256 securityDeposit;     // ton unit
**        uint256 seigs;               // this is seignorage of **operator** 
**    }
```

## F29. Claim to the operator's seignorage  
...
## F30. Stake to the special operator 
...
## F32. Unstake to the special operator 
...
## F34. Re-stake to the special operator 

- contract : OptimismL2Operator
- Interface
```javascript
function restake(uint32 _index, bool isCandidate) public
```
...
## F36. Multi-restake to the special operator 

- contract : OptimismL2Operator
- Interface
```javascript
function restakeMulti(uint32 _index, uint256 n, bool isCandidate) public
```
...
## F38. Withdraw to the special operator 

- contract : OptimismL2Operator
- Interface
```javascript
function withdraw(uint32 _index, bool isCandidate) public
```
...
## F40. Provide the liquidity to fast-withdraw request in special operator/candidate   
...
- isCandidate : if operator, it is false, if candidate, it's true.  
- indexNo :  operatorIndex or candidateIndex 
- layerIndex : operatorIndex for fast withdrawal request
...
_message = abi.encodeWithSignature(
                ...
                ton,     // L1 ton address
                _layerInfo.l2ton, // L2 ton address
                requestor,   // 
...
## F49. Get indexLton at a specific snapshot Id 
...
## F50. Convert  TON amount to LTON
...
## F51.  Convert  TON amount to LTON at a specific snapshot Id 
...
## F52. Convert  LTON amount to TON
...
## F53. Convert  LTON amount to TON at a specific snapshot Id 
...
## F54. Get totalLton amount of staked  
...
## F55. Get  totalLton amount of staked  at a specific snapshot Id
...
## F56. Get total staked Lton of operators   
...
## F57. Get  total staked Lton of operators  at a specific snapshot Id
...
## F58. The staked Lton amount of special operator  at a specific snapshot Id
...
## F59. The staked Lton amount of special candidate  at a specific snapshot Id
...
## F60. The staked Lton amount of special operator's account at a specific snapshot Id
...
## F61. The staked Lton amount of special candidate's account at a specific snapshot Id
...
## F62. The staked TON amount of special operator's account  
...
## F63. The staked TON amount of special operator's account at a specific snapshot Id
...
## F64. The staked TON amount of special candidate's account  
...
## F65. The staked TON amount of special candidate's account at a specific snapshot Id
