...
1. **에폭별 반감기 테이블** — 20개 에폭의 정적 데이터 (현재 에폭 하이라이트)
1. **공급량 진행 바** — MAX_SUPPLY 대비 현재 공급량, 에폭 경계선 마커
1. **공급 곡선 차트** — 누적 raw TON 대비 실제 vTON 공급량 (AreaChart)
1. **반감비율 차트** — 에폭별 계단식 감소 시각화 (LineChart)
