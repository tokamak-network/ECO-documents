...
Current Task for team

- Staking V2 & FW (Modeling, Contract, Service, Design) - since 1Q
- DAO V1 + V2 (Contract, Service) - since 1Q
- new DAO & price dashboard (Modeling, Design) - since end of 3Q
...
- Seig distribution modeling
  - 80%([document](https://docs.google.com/spreadsheets/d/19SEEVLMazPdkdqNJ5dr1dyVhzqT1V71jcNRlhjRuvGY/edit#gid=0)) finished but drop to 10% because of model change, modeling will be finished Q4
- Governance research
  - 80% → 1 article is published to medium, another one is almost finished except conclusion([link](https://docs.google.com/document/d/1hhwE3kgajOMRziEp0qH65njkQzcGT_BUOUYKnpVIBZM/edit#heading=h.sjqoey8dsgdr)), will finished at early Q4
- DAO V1 + V2 contract
  - implementation - implementation is finished (100%), but internal logic will be changed at Q4
  - Audit 
    - external audit & reflect audit (0%) → model change, it might be proceed in Q1, 2024
- Staking migration contract
  - not proceed (0%) → model change & resource issue, this will be proceed at Q4
- Staking storyboard & design
  - not proceed (0%) → model change & resource issue, this work is panned at Q4
- DAO design
  - not proceed (0%) → resource issue, It will be replace with new DAO & price dashboard design work at Q4
- Staking subgraph development
  - not proceed (0%) → replace to publish DAO (70%), and subgraph will implement at Q4
- Unified bridge pool part
  - 100%, but conducted in Tokamak bridge project

## Major issues

### 1. Summary of this quarter

In this quarter layer2 economics team didn’t proceed with many things due to priority issues. So, lots of team members focus on another project. However, some of the team members concentrated on this project to develop our new DAO. The are achievement of this quarter below:

- Develop DAO v1 + v2 contract is finished. And also did internal audit. External audit will be conducted with staking v2 contracts in the future.
  - Progress of contract part
    - Staking & FW → 40% achieved (60%)
    - DAO V1 + V2 → 75% achieved (30%)
- Modeling team started research about new DAO design including price dashboard.
  - Progress of modeling part
    - Staking & FW → 60% achieved (50%)
    - new DAO & price dashboard → 40%(10%)
- And service team is porting old DAO UI from Vue to React. It is not connected with smart contract.
  - Progress of service part
    - Staking & FW → 50% achieved (50%)
    - DAO V1 & V2 → 40% achieved (10%)

### 2. Seigniorage distribution model changes

The seigniorage distribution model will be changed once again. In the last quarter, the seigniorage distribution model was changed, because we have to distribute TON both V1 & V2 staker. So, we try to distribute TON by using distribute for DAO. But we have to reconsider another model because it has other obstacles
...
| Q3 Budget | 3000(L2) |  |  |
| BOE |  | 1,136 |  |
| Total | **74,538** | **69,819** | **4,764(L1+L2)** |

## Q3 2023: What did we do?

## output link: 

[L2 Economics Reporting](https://docs.google.com/spreadsheets/d/1dTV7H5ipGQWVrBd58S98_2mFtKcHfHqZ9g2BuvZcjxI/edit#gid=870905664)
...
- In Q4, L2 Economics team aims three main goals.
  - Patch Staking & DAO with fixed contract
  - Revamping price dashboard
  - Start to improve Staking V2 with FW
- Task of next quarter
  - Staking V2 & FW (Modeling, Contract, Service, Design)
  - DAO V1 + V2 (Contract)
  - new DAO & price dashboard (Modeling, Design, Service)
  - patch Staking V1 (Modeling, Contract, Service)
